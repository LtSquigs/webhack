// Modified HTERM with the following changes
// Line 5117 - Added code to call global update cursor function
// Line 14527 - Added code to the ANSI J escape code handling to clear tile window
// Line 14711 - Added code to handle the ANSI z escape code from vt_tiledata
...
hterm.RowCol.prototype.move = function(row, column, opt_overflow) {
  ...
  window.updateTileCursor(row, column);
};
...
/**
* Handle the z escape codes that NAO sends for vt_tiledata
*/
hterm.VT.CSI['z'] = function(parseState) {
  this.terminal.getCursorColumn();
  this.terminal.getCursorRow(); // Send Signal to draw tile on canvas?

  // 0 Means a tile needs to be drawn, send coordinates and id
  if((parseState.args[0] * 1) === 0) {
    window.drawTile(this.terminal.getCursorRow() * 1, this.terminal.getCursorColumn() * 1, parseState.args[1] * 1);
  } else if ((parseState.args[0] * 1) === 2) { // 2 Means window changed

    // If we're changing from an old window, send old window to update function
    if(window.currentWindow !== parseState.args[1] * 1)
    {
      window.updateWindowFunction(window.currentWindow, parseState);
    }

    // Update current window
    window.currentWindow = parseState.args[1] * 1;
  } else if ((parseState.args[0] * 1) === 3) { // 3 Means nethack is waiting for input
    // Send current window to update function
    window.updateWindowFunction(window.currentWindow, parseState);
  }
};
...
'af9821017716bc8f341aea678518c0a7f748ee89' +
''
);
